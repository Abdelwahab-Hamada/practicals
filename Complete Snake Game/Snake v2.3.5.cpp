...
class SNAKE
{
  ...
  public:
	  SNAKE(int width,int height)
	{
		...
		(width/ 2,height/2);
		body.push_back(std::make_shared<TAIL> 
	...
	void shift(std::vector<std::shared_ptr
	<CELL>> cells,std::shared_ptr<CELL> 
	followed)
	{
		// original old
		...
		cells.size();index++)
		{
			// old
			prevXI 
			=cells.at(index).get()->row;
			prevYI  
			=cells.at(index).get()->col;
			dirII  
			=cells.at(index).get()->dir;
...
class GAME_ENGINE_IF
{
  ...
  protected:
	...
	void grow()
	{
		...
		else
		{
			...
			snakeBody.push_back
			(std::make_shared<TAIL>				    (snakeBody.back()->XY().first, 
  ...
  public:
	virtual void 
	attach(std::shared_ptr<BOARD_IF>) =0;

	virtual void 
	dettach(std::shared_ptr<BOARD_IF>) =0;
...
class GAME_ENGINE:public GAME_ENGINE_IF
{
  ...
  public:
	...
	std::shared_ptr<CELL> snakeHead,
	std::vector<std::shared_ptr<CELL>> 
	snakeBody,std::shared_ptr<CELL> 
	element,bool autoMove,bool chase)
	{
		this->score = score;
	...
	void setChangeable()
	{
		...
		snakeBody, element, autoMove, chase);
	}
...
class BOARD:public BOARD_IF, public DISPLAYBOARD_IF
{
	  ...
	  std::string autoMoveState;
	  std::string chaseState;
	  std::string speedState;
	  bool autoMove,chase;
	  /*enum eSpeed
  	{ fast = 10000, slow = 100000 };
  	eSpeed speed;*/ 
  ...
  public:
	  BOARD(std::shared_ptr < GAME_ENGINE > gameEngine, int w, int h):gameEngine_(gameEngine)
	{
		this->width = w;
		this->height = h;
		gameEngine_->attach(std::shared_ptr 
		<BOARD_IF> (this));
		//this->autoMoveState 
		//="\x1B[90mMove\033[0m";
		//this->chaseState 
		//="\x1B[90mChase\033[0m";
		//this->speedState ="Stop";
	}
